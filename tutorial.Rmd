...
## Step by step breakdown of CART:
1. Start with full dataset as the 'root' of the tree
2. a) Identify which node(s) should be split 
   b) which the feature(s) by which the splitting(s) should be done
   - for a classification tree, we compute the gini index for each feature, then select the one with the lowest gini index
   - for a regression tree, we compute the SSE for each possible split, then choose the feature with the lowest SSE
3. Split the dataset accordingly
4. Examine the children - determine if they are 'leaves' or need to be split further
  - a node is deemed terminal (a leaf) if it has less than a certain number of observations, or if it contains observations with the same label.
...
Change the status of the current from "split" to "parent":
```{r}
...
The final outputs after the tree is fully grown is as follows:
```{r}
output
```
...
```{r}
classtree(data = data, resp = "readm", min.obs = 3)
```

We can check the function against package 'rpart':

```{r}
library(rpart)
rpart(readm ~., data = data,  minsplit = 3)
```


Test using the dataset 'kyphosis':

```{r}
classtree(data = kyphosis, resp = "Kyphosis", min.obs = 20)$output

rpart(Kyphosis ~., data = kyphosis, control = rpart.control(xval=0,minbucket=1,minsplit = 20, cp = -1))
```
...
Check the function against package 'rpart':

```{r}
rpart(time ~., data = data, minsplit = 3)
```

Using the first 200 obs. of dataset 'ML_reg_2":
```{r}
data = read.csv("Datasets/ML_reg 2.csv", row.names = 1)[1:200,]
rpart(num ~., data = data, minsplit = 40, minbucket=4)
regtree(data=data, resp = "num", min.obs = 40)$output
```