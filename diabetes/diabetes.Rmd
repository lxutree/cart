...
```{r}
set.seed(123)
tree_rf <- train(readmitted ~ discharge + race + source + medical_specialty + time_in_hospital + age + diag_1 + A1Cresult, data = data, 
                 method = "rf",
                 trControl = trCntl)

roc_rf <- roc(tree_rf$pred$obs, tree_rf$pred$YES,
            # arguments for ci
            ci=TRUE, plot = TRUE, auc = TRUE)
roc_rf

cal_rf = val.prob(tree_rf$pred$YES, ifelse(tree_rf$pred$obs == "NO", 0,1), logistic.cal = FALSE, lim = c(0,0.4))
...
| Type              | AUC           | Intercept | Slope |
|------------------:|:-------------:|:---------:|:-----:|
|Logistic regression| 0.6105        | -0.306    | 0.863 |
|Classification Tree|  0.5693       |  -1.471   | 0.362 |
|Bagging            | 0.606         | -0.175    | 0.923 |
|Random Forest      |    0.5228     |  -2.064   | 0.0366|
|Boosting           |   0.607       |    0.050  |  1.022|
