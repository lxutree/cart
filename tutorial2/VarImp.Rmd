...
res = regtree(data = rhcdata, resp = "Y", min.obs = min.obs, feat = c("Disease.category", "Albumin", "Cardiovascular"))
```
...
```{r}
res_rpart = rpart(Y~Disease.category+Albumin+Cardiovascular, data = rhcdata, minsplit = min.obs, cp=-1)
res_rpart
...
```{r}
tree = rpart(resid ~ ., data = data_boost, minsplit = min.obs, cp = -1)
tree
```
...
```{r}
gbmboost <- gbm(Y ∼ ., data = rhcdata, distribution = "gaussian", n.trees = 2, n.minobsinnode = min.obs, bag.fraction = 1, shrinkage=0.1)
